...
import com.nexapp.nexmusic.constants.ShowAudioFallbackToastKey
...
highlightKey: String? = null) {
    ...
    if (showDownloadQualityDialog) {
        EnumDialog(
            ...
            values = listOf(com.nexapp.nexmusic.constants.DownloadQuality.YOUTUBE, com.nexapp.nexmusic.constants.DownloadQuality.SAAVN),
            valueText = {
                when (it) {
                    com.nexapp.nexmusic.constants.DownloadQuality.YOUTUBE -> "YouTube Music (AAC/Default)"
                    com.nexapp.nexmusic.constants.DownloadQuality.SAAVN -> "Saavn (320kbps)"
                    com.nexapp.nexmusic.constants.DownloadQuality.LOSSLESS -> "Qobuz (Lossless)"
                }
            }
        )
    }
    ...
    Column(
        ...
        if (showSaavnAudioWarning) {
            DefaultDialog(
                ...
                Text("Saavn (320kbps) streams run through third-party servers and cost real money to keep running.\n\nNote: If Saavn playback fails, the app automatically falls back to YouTube Music's Opus stream.")
            }
        }
        ...
        if (showLosslessAudioWarning) {
            DefaultDialog(
                ...
                Text("Lossless (Qobuz) streams run through third-party servers and cost real money to keep running.\n\n" + stringResource(R.string.lossless_audio_warning))
            }
        }
