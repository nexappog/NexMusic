...
object YTPlayerUtils {
    ...
    /**
     * Warms up the WebView-based PoToken generator in the background so the first real
     * song play doesn't have to pay the full cold-start cost (loading a page + running
     * botguard JS to solve YouTube's bot-check, which can take several seconds). Safe to
     * call speculatively — failures are swallowed since this is a best-effort perf
     * optimization; the real per-song call will still work correctly (just slower) if
     * this hasn't completed yet or failed.
     */
    suspend fun warmUpPoTokenGenerator() {
        val sessionId = YouTube.dataSyncId ?: YouTube.visitorData ?: return
        withContext(Dispatchers.IO) {
            try {
                Timber.tag(logTag).d("Warming up PoToken generator in background")
                poTokenGenerator.getWebClientPoToken("dQw4w9WgXcQ", sessionId)
                Timber.tag(logTag).d("PoToken generator warm-up complete")
            } catch (e: Exception) {
                Timber.tag(logTag).d("PoToken warm-up skipped/failed (non-fatal): ${e.message}")
            }
        }
    }
    ...
    suspend fun playerResponseForPlayback(
        ...
        val showFallbackToast = context?.let { 
            it.dataStore.data.first()[com.nexapp.nexmusic.constants.ShowAudioFallbackToastKey] 
        } ?: true
                ...
                try {
                    qobuzAttempt = kotlinx.coroutines.withTimeoutOrNull(15000L) {
                        ...
                        if (title != null && author != null) {
                            val qobuzClient = com.nexapp.nexmusic.utils.qobuz.QobuzApiClient()
    ...
    private suspend fun resolvePlaybackData(
        ...
        // MAIN_CLIENT, METADATA_CLIENT and Signature resolution are independent of
        // each other, so they run concurrently instead of one after another. PoToken
        // generation is still internally serialized by PoTokenGenerator's mutex (only one
        // WebView JS context), but each client's network request now fires as soon as its
        // own PoToken is ready rather than waiting for other resolution round trips
        // to finish first — this roughly halves perceived load time for logged-in users.
        ...
        coroutineScope {
            val signatureTimestampDeferred = async { getSignatureTimestampOrNull(videoId) }

            val mainDeferred = async {
                val sigTs = signatureTimestampDeferred.await()
                sessionId = if (isLoggedIn) YouTube.dataSyncId else YouTube.visitorData
                ...
                YouTube.player(videoId, playlistId, MAIN_CLIENT, sigTs.timestamp, poToken?.playerRequestPoToken).getOrThrow()
            }
                ...
                async {
                    ...
                    try {
                        ...
                        val response = YouTube.player(
                            ...
                            sigTs.timestamp, metaPoToken?.playerRequestPoToken
                        ).getOrNull()
            ...
            signatureTimestampResult = signatureTimestampDeferred.await()
        }
        ...
        wasOriginallyAgeRestricted = isAgeRestrictedFromResponse || (signatureTimestampResult?.isAgeRestricted == true)
        ...
        for (clientIndex in (startIndex until STREAM_FALLBACK_CLIENTS.size)) {
            ...
            if (clientIndex == -1) {
                ...
                val fallbackSessionId = sessionId
                if (client.useWebPoTokens && poToken == null && fallbackSessionId != null) {
                    Timber.tag(logTag).d("Lazily generating PoToken for fallback web client: ${client.clientName}")
                    try {
                        poToken = poTokenGenerator.getWebClientPoToken(videoId, fallbackSessionId)
                    } catch (e: Exception) {
                ...
                val clientSigTimestamp = if (wasOriginallyAgeRestricted) null else signatureTimestampResult?.timestamp
            ...
            if (streamPlayerResponse?.playabilityStatus?.status == "OK") {
                ...
                if (currentClient.useWebPoTokens && currentPoToken?.streamingDataPoToken != null) {
                    Timber.tag(logTag).d("Appending pot= parameter to stream URL")
                    val separator = if ("?" in streamUrl!!) "&" else "?"
                    streamUrl = "${streamUrl}${separator}pot=${currentPoToken.streamingDataPoToken}"
                }
...
fun confidence(queryArtist: String, queryTitle: String, queryDuration: Long?, candidate: com.nexapp.nexmusic.utils.qobuz.QobuzTrack): Float {
